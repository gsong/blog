...
date: 2020-07-19T06:22:41-07:00
keywords:
  - javascript
  - react
---
...
This is one key difference compared to `useState`. The `useReducer` hook
computes the next state internally using a `reducer` function. The `useState`
hook relies entirely on external calculations for its next state.

Let's focus on different `reducer` (a.k.a. "new state" calculator)
...
As you can see, the names `state`, `dispatch`, and `reducer` are conceptual. In
practice, you can name them anything you like. In this example, they are `date`
(state), `update` (dispatch), and `reducer`.

Also notice that when we invoke `update()`, we're doing it _without_ passing in
any parameters. `dispatch` signals the hook to run the reducer to compute the
new state, it's up to you whether you need to send an action with the signal or
not. In this case, the hook actually runs `reducer(currentState, undefined)`.
